...
2. Load `{project-root}/_bmad/config.yaml` (and `config.user.yaml` if present): greet with `{user_name}`, speak in `{communication_language}`, and resolve `{output_folder}` and `{date}`.
