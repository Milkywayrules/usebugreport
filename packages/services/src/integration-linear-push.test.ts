...
import {
  LINEAR_PUSH_ACTION,
  createLinearPushHandlers,
} from "./integration-linear-push";
...
describe("integration linear push", () => {
  ...
  test("pending duplicate does not enqueue a second job", async () => {
    ...
    let enqueueCount = 0;
    const handlers = createLinearPushHandlers(
      mock.db,
      {
        appUrl: "https://app.example",
        enqueueLinearPush: async () => {
          enqueueCount += 1;
        },
      },
      internals
    ...
    const result = await handlers.pushReportToLinear(ctx, "rpt_test");
  ...
  test("failed push requires explicit retry", async () => {
    ...
    const handlers = createLinearPushHandlers(
      ...
      {
        appUrl: "https://app.example",
        enqueueLinearPush: async () => undefined,
      },
      internals
    );

    await expect(handlers.pushReportToLinear(ctx, "rpt_test")).rejects.toMatchObject({
      code: "CONFLICT",
    });
  });
